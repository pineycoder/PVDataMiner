...
prefix = Optional(I('a')).hide() + (Optional(lbrct) + W('JSC') + Optional(rbrct)| R('mA', re.I) | (I('short') + I('circuit') | I('current') + I('density')) | R('mA', re.I) + R('^pt?\.?$', re.I)).hide() + Optional(lbrct + W('JSC') + rbrct) + Optional(W('=') | I('of') | I('was') | I('is') | I('at')).hide() + Optional(I('in') + I('the') + I('range') + Optional(I('of')) | I('about')).hide()

delim = R('^[:;\.,]$')

# TODO: Consider allowing degree symbol to be optional. The prefix should be restrictive enough to stop false positives.
units = (W('mA cm') + Optional(R('^[CFK]\.?$')) | W('mA'))('units').add_action(merge)
...
jsc_phrase = cem_mp_phrase | to_give_mp_phrase | obtained_mp_phrase
