...
prefix = Optional(I('a')).hide() + (Optional(lbrct) + Optional(W('Eg')) + Optional(rbrct)| R('eV', re.I) | (Optional(I('energy')) +I('band') + I('gap')) | R('eV', re.I) + R('^pt?\.?$', re.I)).hide() + Optional(lbrct + W('Eg') + rbrct) + Optional(W('=') | I('of') | I('was') | I('is') | I('at')).hide() + Optional(I('in') + I('the') + I('range') + Optional(I('of')) | I('about')).hide()

delim = R('^[:;\.,]$')

# TODO: Consider allowing degree symbol to be optional. The prefix should be restrictive enough to stop false positives.
units = (W('eV') + Optional(R('^[CFK]\.?$')) | W('J'))('units').add_action(merge)
