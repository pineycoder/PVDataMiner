...
prefix = Optional(I('a')).hide() + (Optional(lbrct) + Optional(W('PCE')) + Optional(rbrct)| R('%', re.I) | (Optional(I('power')) +I('conversion') + I('efficiency')) | R('%', re.I) + R('^pt?\.?$', re.I)).hide() + Optional(lbrct + W('PCE') + rbrct) + Optional(W('=') | I('of') | I('was') | I('is') | I('at')).hide() + Optional(I('in') + I('the') + I('range') + Optional(I('of')) | I('about')).hide()

delim = R('^[:;\.,]$')

# TODO: Consider allowing degree symbol to be optional. The prefix should be restrictive enough to stop false positives.
units = (W('%') + Optional(R('^[CFK]\.?$')) | W('%'))('units').add_action(merge)
