...
prefix = Optional(I('a')).hide() + (Optional(lbrct) + W('VOC') + Optional(rbrct)| R('V', re.I) | I('voltage') + Optional((I('open') | I('circuit')| I('voltage'))) | R('V', re.I) + R('^pt?\.?$', re.I)).hide() + Optional(lbrct + W('VOC') + rbrct) + Optional(W('=') | I('of') | I('was') | I('is') | I('at')).hide() + Optional(I('in') + I('the') + I('range') + Optional(I('of')) | I('about')).hide()

delim = R('^[:;\.,]$')

# TODO: Consider allowing degree symbol to be optional. The prefix should be restrictive enough to stop false positives.
units = (W('V') + Optional(R('^[CFK]\.?$')) | W('K\.?'))('units').add_action(merge)
